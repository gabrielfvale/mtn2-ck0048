...
met_jacobi = mj.metodo_de_jacobi(A, 5, 0.000001)

print('Questão 1')

print('i)\n ------- Matriz Diagonal -------')
print(np.diag(met_jacobi[1]))

print('\nii)\n ------- Matriz Acumulada P -------')
print(np.matrix(met_jacobi[0]))

print('\niii)\n ------- Matrizes de Cada Varredura -------')
for matriz in met_jacobi[2]:
    print(matriz, '\n')

print('\niv)\n ------- Pares (Autovalor, Autovetor) de A -------')
for i in range (5):
    autovalor = met_jacobi[1][i]
    autovetor = np.transpose(met_jacobi[0])[i]
...
matriz_resultante = mj.metodo_de_jacobi(matriz_tridiagonal, 5, 0.000001)[0]
array_autovalores = mj.metodo_de_jacobi(matriz_tridiagonal, 5, 0.000001)[1]
